...
real(kind=8),parameter :: dt=.005d0,dx=1.d0,dy=1.d0
...
! Impose periodic boundary conditions
	...
	dgdc=.5d0*W*(Conc_Dom2-CmBt)*Conc_Dom2**2.d0+.5d0*W*Conc_Dom2*(Conc_Dom2-CmBt)**2.d0
...
implicit none
! Impose periodic boundary conditions
	real(kind=DBL), DIMENSION(0:nx+1,0:ny/2+1) :: Pot_Dom1 
	real(kind=DBL), DIMENSION(0:nx+1,ny/2:ny+1) :: Conc_Dom2, Pot_Dom2
	integer :: i,j	
	...
	do i=1,nx
		if (Conc_Dom2(i,ny/2+1) .gt. 0.5) then
			Pot_Dom1(i,ny/2+1)=Pot_Dom2(i,ny/2+1)
			Pot_Dom2(i,ny/2)=Pot_Dom1(i,ny/2)
		else
		Pot_Dom1(i,ny/2+1)=Pot_Dom1(i,ny/2)
...
!*********************************************************************
!********************************************************************* 	
subroutine write_output(Conc_Dom1,Conc_Dom2,Pot_Dom1,Pot_Dom2,iter)
use simulation
implicit none
	real(kind=DBL), DIMENSION(0:nx+1,0:ny+1) :: Conc
	real(kind=DBL), DIMENSION(0:nx+1,0:ny/2+1) :: Conc_Dom1,Pot_Dom1
	real(kind=DBL), DIMENSION(0:nx+1,ny/2:ny+1) :: Conc_Dom2,Pot_Dom2
	INTEGER :: iter
