function main
    ...
    %grid spacing
    dx=1.0;
    %inter-diffusivit mobility for the concentration
    D=1.0;
    ...
    phi=0.5*(1+tanh(2.0*(x_coord+10)/delta));
    phi1=0.5*(1-tanh(2.0*(x_coord-10)/delta));
    phi=phi.*phi1;
...
function ans=make_plots(x_coord,phi,Conc,iter)
    ...
    title(sprintf('iter=%d',iter),'fontsize',25)
...
function ans=chem_pot(phi,Conc,dx,W,eps2,A0Bt,A1Bt,CmBt,A0Al,A1Al,CmAl)
    ...
    ans=dgdphi(phi,W)-eps2*lap1D(phi,dx)...
    +dHfunc_dphi(phi).*(GBt-GAl-(ConcBt-ConcAl).*dG_dCAl);
end
function phi=AC_iter(phi,Conc,dt,dx,M,W,eps2,A0Bt,A1Bt,CmBt,A0Al,A1Al,CmAl)
    % Explicit time iteration of the Allen-Cahn equation    
    phi=phi-M*chem_pot(phi,Conc,dx,W,eps2,A0Bt,A1Bt,CmBt,A0Al,A1Al,CmAl)*dt;
end
function Conc=diff_iter(phi,Conc,D,dt,dx,W,eps2,A0Bt,A1Bt,CmBt,A0Al,A1Al,CmAl)
